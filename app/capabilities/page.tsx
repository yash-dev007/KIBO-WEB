...
export default function Capabilities() {
  return (
    <section className="relative py-[80px] sm:py-[90px] lg:py-[130px]" id="capabilities">
      <div className="max-w-[1360px] px-4 sm:px-6 md:px-8 lg:px-[64px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[50px] md:gap-[70px] items-center">
          <Reveal delay={0} direction="left">
            <div className="relative aspect-square max-w-[600px] mx-auto md:mx-0">
          ...
          <div className="flex flex-col">
            ...
            <Reveal delay={80}>
              <h2 className="font-sans font-extrabold tracking-[-0.028em] text-ink leading-none text-[clamp(34px,4.8vw,64px)] my-[22px] md:mb-[30px]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-[18px] mt-[22px]">
              {[
                { n: "01", tag: "Observation", icon: <><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="4"/></>, title: "Persistent\nSight", desc: "Optional screen and camera awareness to understand your physical and digital environment.", label: "Learn more about Observation" },
                { n: "02", tag: "Memory", icon: <path d="M21 8v8a5 5 0 0 1-5 5H8a5 5 0 0 1-5-5V8a5 5 0 0 1 5-5h8a5 5 0 0 1 5 5z"/>, title: "Infinite\nRecall", desc: "Every interaction is indexed. KIBO remembers that color you liked three weeks ago.", label: "Learn more about Memory" },
                { n: "03", tag: "Reaction", icon: <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/>, title: "Proactive\nAction", desc: "KIBO doesn't wait for a prompt. It suggests files, snippets, and ideas as you work.", label: "Learn more about Reaction" },
                { n: "04", tag: "Local", icon: <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>, title: "Privacy\nFirst", desc: "All data stays on your machine. Local-first intelligence means zero-cloud dependency.", label: "Learn more about Local" },
              ].map((card) => (
                <Reveal key={card.n} delay={parseInt(card.n) * 90}>
                  <div className="p-[28px_26px_32px] bg-bone rounded-[18px] shadow-[0_30px_60px_-30px_rgba(21,20,15,0.18),inset_0_0_0_1px_rgba(21,20,15,0.06)] relative overflow-hidden transition-transform duration-200 hover:-translate-y-[3px] group">
                    <div className="font-serif italic text-[22px] font-medium text-coral tracking-[0.04em] mb-[16px] flex justify-between items-baseline">
                      {card.n}<span className="font-sans text-[9.5px] text-ink-faint tracking-[0.18em] uppercase not-italic font-medium">{card.tag}</span>
                    </div>
                    <svg className="w-[28px] h-[28px] mb-[16px] text-ink" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">{card.icon}</svg>
                    <h3 className="font-sans text-[22px] font-bold leading-[1.05] tracking-[-0.014em] mb-[14px]">{card.title.split("\n").map((l, i) => <span key={i}>{l}{i === 0 && <br/>}</span>)}</h3>
                    <p className="font-body text-[13.5px] text-ink-mute leading-[1.55] max-w-[24ch]">{card.desc}</p>
                    <a className="absolute right-[22px] bottom-[22px] w-[28px] h-[28px] border border-[var(--line)] rounded-full inline-flex items-center justify-center text-ink transition-all duration-180 group-hover:bg-coral group-hover:border-coral group-hover:text-white" href="#" aria-label={card.label}>
